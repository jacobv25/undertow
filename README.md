...
```mermaid
flowchart LR
    A["Instagram · Threads · Reddit<br>TikTok · YouTube · Facebook"] -- "TYPE_VIEW_SCROLLED" --> B["ScrollWatcherService<br>(AccessibilityService)"]
...
- **`ScrollWatcherService`** — an `AccessibilityService` receiving
  ...
  (Instagram, Threads, Reddit, Facebook, TikTok, YouTube). For YouTube, only scrolling inside the
